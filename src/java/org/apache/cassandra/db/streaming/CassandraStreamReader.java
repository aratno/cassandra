...
public class CassandraStreamReader implements IStreamReader
{
    ...
    protected SSTableMultiWriter createWriter(ColumnFamilyStore cfs, long totalSize, long repairedAt, TimeUUID pendingRepair, ImmutableCoordinatorLogOffsets coordinatorLogOffsets, SSTableFormat<?, ?> format) throws IOException
    {
        ...
        if (isTracked)
        {
            ...
            return new RangeAwareSSTableWriter(cfs, estimatedKeys, repairedAt, pendingRepair, false, coordinatorLogOffsets, format, sstableLevel, totalSize, lifecycleNewTracker, getHeader(cfs.metadata()));
    }
